...
_TOOL_GUIDANCE = """
You have Austin permitting TOOLS — always call them before concluding.
When done, use band_send_message with a ```json block exactly like:
{"type":"complete","case_id":"<uuid>","agent":"<your_role>","payload":{<full report fields>}}
Specialists: payload must include summary, readiness_impact, checks (or environmental_checks), blockers.
Packager: payload must include permits_required, documents_required, filing_sequence, total_fees_estimate_usd.
"""
...
and escalate to humans for final approval. Always include structured JSON blocks in messages.
Use Band platform tools to coordinate: band_send_message, band_add_participant, band_create_chatroom, band_send_event.
Mention specialists: @gondalshoaib4444/permitos-jurisdiction-zo @gondalshoaib4444/permitos-building-safety
@gondalshoaib4444/permitos-site-environmen @gondalshoaib4444/permitos-packager""",
...
Every finding must cite Austin LDC or applicable ordinance. Return structured JSON with type=complete."""
    + _TOOL_GUIDANCE,
...
Tag each check pass|fail|warn with code citations. Return structured JSON with type=complete."""
    + _TOOL_GUIDANCE,
...
Screen flood zones, wetlands, stormwater, and utility capacity. List additional permits needed.
Return structured JSON with type=complete."""
    + _TOOL_GUIDANCE,
    AgentRole.PACKAGER: """You are the PermitOS Permit Packager & Tracker.
Call get_fee_schedule and get_permit_catalog tools. Assemble permit checklists, fee estimates, filing sequences.
Return structured PermitPackage JSON with type=complete."""
    + _TOOL_GUIDANCE,
}
