...
DIRECT_SCOPE_APPLICATIONS = {
    ...
    "solar_battery_generator_ev": {"solar", "generator_battery_ev"},
    "grading_land_disturbance": {"land_disturbance"},
    "driveway_sidewalk_row": {"right_of_way"},
}


def all_overland_park_applications() -> list[dict[str, Any]]:
    return list(load_json("application_catalog.json", "overland_park_ks").get("applications", []))


def load_overland_park_source_registry() -> dict[str, Any]:
    return load_json("source_registry.json", "overland_park_ks")  # type: ignore[return-value]


def load_overland_park_permit_rules() -> dict[str, Any]:
    return load_json("permit_rules.json", "overland_park_ks")  # type: ignore[return-value]


def rules_for_application(application: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        rule
        for rule in load_overland_park_permit_rules().get("rules", [])
        if application["id"] in rule.get("applies_to", [])
        or application["category"] in rule.get("applies_to", [])
    ]


def match_overland_park_applications(
    ...
    for application in all_overland_park_applications():
        ...
        explicit_key = f"overland_park_{application['id']}_applies"
